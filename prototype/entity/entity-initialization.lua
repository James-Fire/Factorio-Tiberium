require("prototype.entity.tiberium-node-harvester")
require("prototype.entity.tiberium-centrifuge")
require("prototype.entity.tiberium-growth-accelerator")
require("prototype.entity.tiberium-obelisk-of-light")
require("prototype.entity.tiberium-advanced-guard-tower")
require("prototype.entity.tiberium-network-node")
require("prototype.entity.tiberium-power-plant")
require("prototype.entity.tiberium-reprocessor")
require("prototype.entity.tiberium-spike")
require("prototype.entity.tiberium-srf")
require("prototype.entity.tiberium-control-node")
require("prototype.entity.tiberium-aoe-node-harvester")
require("prototype.entity.tiberium-sonic-emitter")
require("prototype.entity.tiberium-detonation-charge")
require("prototype.entity.tiberium-monoculture")
