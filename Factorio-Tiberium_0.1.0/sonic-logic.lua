...
local kw_per_tick = 45
local kj_to_fill = 25000
local wall_health = 10000

local joules_per_hitpoint = 400
local node_range = 15

function findNodes(surf, pos, force, dir)
...
function wallDamage(surf, pos, tick)
    if global.hexi_hardlight_segments[surf.index] and global.hexi_hardlight_segments[surf.index][math.floor(pos.x)] and global.hexi_hardlight_segments[surf.index][math.floor(pos.x)][math.floor(pos.y)] then
        ...
        local connected_nodes = findNodes(surf, pos, nil, wall[1], true)
        ...
        for k,v in pairs(connected_nodes) do
            ...
            if energy == 0 then
                if not mark_death then
                    sendAlert(v.force, defines.alert_type.entity_destroyed, wall[2])
                    mark_death = true
                end
                disableNode(v)
        ...
        if not mark_death then
            sendAlert(force, defines.alert_type.entity_under_attack, wall[2])
        end
    end
end

function sendAlert(force, alert_type, entity)
    for k,v in pairs(force.players) do
        v.add_alert(entity, alert_type)
    end
end

function testWall(surf, pos, dir, node)
    ...
    if not global.hexi_hardlight_segments[surf.index][math.floor(pos[1])][math.floor(pos[2])] then
        if not surf.can_place_entity{name="sonic-wall", position=pos, force=node.force} then return false end
    else
...
function makeWall(surf, pos, dir, node)
    if not global.hexi_hardlight_segments[surf.index] then global.hexi_hardlight_segments[surf.index] = {} end
    if not global.hexi_hardlight_segments[surf.index][math.floor(pos[1])] then global.hexi_hardlight_segments[surf.index][math.floor(pos[1])] = {} end
    
    if not global.hexi_hardlight_segments[surf.index][math.floor(pos[1])][math.floor(pos[2])] then
        local wall = surf.create_entity{name="sonic-wall", position=pos, force=node.force}
...
function tryMakeWall(node1, node2)
    ...
    if node1.position.x == that_pos.x and node1.position.y ~= that_pos.y then
        ...
        if math.abs(diff) <= node_range and math.abs(diff) > 1 then
            ...
            for y=sy,ty do
                if not testWall(node1.surface, {node1.position.x,y}, vert_wall, node1) then return end
            end
            for y=sy,ty do
                makeWall(node1.surface, {node1.position.x,y}, vert_wall, node1)
            end
        end
    elseif node1.position.x ~= that_pos.x and node1.position.y == that_pos.y then
        ...
        if math.abs(diff) <= node_range and math.abs(diff) > 1 then
            ...
            for x=sx,tx do
                if not testWall(node1.surface, {x,node1.position.y}, horz_wall, node1) then return end
            end
            for x=sx,tx do
                makeWall(node1.surface, {x,node1.position.y}, horz_wall, node1)
            end
        end
    end
end
...
script.on_event(defines.events.on_tick, function(event)
    ...
    while #global.hexi_hardlight_damage > 0 do
        wallDamage(global.hexi_hardlight_damage[1][1], global.hexi_hardlight_damage[1][2], event.tick)
    ...
    for i=#global.hexi_hardlight_node_ticklist,1,-1 do
        ...
        if cur[2] <= cur_tick then
            ...
            if charge_rem <= 0 then
                local connected_nodes = findNodes(cur[1].surface, cur[1].position, cur[1].force, horz_wall + vert_wall)
                for k,v in pairs(connected_nodes) do
...
script.on_event(defines.events.on_trigger_created_entity, function(event)
    if event.entity.name == "sonic-wall-damage" then
...
script.on_event(defines.events.on_built_entity, function(event)
    if event.created_entity.name == "sonic-wall-node" then addNode(event.created_entity, event.tick) end
end)
...
script.on_event(defines.events.on_pre_player_mined_item, function(event)
    if event.entity.name == "sonic-wall-node" then delNode(event.entity) end
end)
